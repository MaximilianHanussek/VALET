...
search_line_number=$(grep -n "master-instance" terraform/terraform.tfstate | cut -f1 -d:)					#Get line after that the master section starts

master_node_IP=$(tail -n +$search_line_number terraform/terraform.tfstate | grep "access_ip_v4" | awk -F ': "' '{print $2}' | head -c -3)	#Get public master node IP
